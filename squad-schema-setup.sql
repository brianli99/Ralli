-- ============================================================================
-- RALLI SQUAD SYSTEM - COMPLETE DATABASE SCHEMA
-- ============================================================================
-- Features: Squads, Friends, Matches, Real-time Messaging (Squad Chat + DMs)
-- Sport-specific templates, customizable squad pages, mobile-optimized
-- ============================================================================
...
-- Core squads table with sport-specific customization
CREATE TABLE IF NOT EXISTS squads (
  ...
  avatar_url TEXT,
  banner_url TEXT,
  theme_color TEXT DEFAULT '#1a73e8', -- Customizable squad color
...
-- Squad membership with roles and sport-specific metadata
CREATE TABLE IF NOT EXISTS squad_members (
  squad_id UUID REFERENCES squads(id) ON DELETE CASCADE,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  role TEXT NOT NULL DEFAULT 'member' CHECK (role IN ('owner','admin','member')),
  position TEXT, -- Sport-specific positions (e.g., 'point guard', 'setter')
...
-- Enhanced matches table with sport-specific customization
CREATE TABLE IF NOT EXISTS matches (
  ...
  -- Location (can reference Google Places or be custom)
  facility_id TEXT, -- Google Places ID
  facility_name TEXT,
  ...
  sport_settings JSONB DEFAULT '{}', -- e.g., {"sets": 3, "games_per_set": 6} for tennis
  ...
  result JSONB, -- Store match results
  created_by UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Match participants (can be squads or individual users)
CREATE TABLE IF NOT EXISTS match_participants (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  match_id UUID REFERENCES matches(id) ON DELETE CASCADE,
  squad_id UUID REFERENCES squads(id) ON DELETE CASCADE,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  team_side INTEGER DEFAULT 1, -- 1 or 2 for team sports
...
CREATE TABLE IF NOT EXISTS squad_messages (
  ...
  metadata JSONB DEFAULT '{}', -- For attachments, match references, etc.
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Direct message threads (normalized to ensure consistent ordering)
...
-- User QR codes for friend discovery
...
-- Users can view squads they're members of or public squads
...
-- Only owners and admins can update squads
...
-- Users can leave squads, owners/admins can remove members
...
-- Squad messages: visible to squad members
...
-- Users can send direct messages in their threads
...
-- Insert default sport templates (optional - can be handled in app)
COMMENT ON TABLE squads IS 'Sport-specific squads with customizable themes and templates';
COMMENT ON TABLE squad_messages IS 'Real-time squad chat with sport context';
COMMENT ON TABLE matches IS 'Sport-specific match scheduling with flexible settings';
COMMENT ON COLUMN matches.sport_settings IS 'JSON field for sport-specific match configuration';
COMMENT ON COLUMN squads.theme_color IS 'Customizable squad theme color';
